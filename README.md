...
- 模式能力由服务端启动档位固定（取消前端运行时切换）：
  - 聊天请求支持 `enable_thinking`、`enable_tool_calling`、`response_format(json_schema)`
  - 聊天输入框保留 Thinking / Tool Calling 开关按钮（仅在服务端支持时生效）
  - Tool Calling 当前内置工具：`get_current_time`、`get_beijing_time`、`math_calculator`、`get_weather`、`calculate_wind_chill`、`search_news`、`search_web_realtime`、`get_exchange_rate`、`stock_quote`、`convert_unit`、`get_calendar_info`、`text_stats`、`generate_uuid`、`random_number`、`coin_flip`、`roll_dice`、`generate_password`、`hash_text`、`base64_codec`
- Gemma4 结构化输出接入优化（vLLM guided decoding）：
  - 聊天请求新增 OpenAI-compatible `response_format` 透传（当前支持 `type=json_schema`）
  - 非流式与流式都可启用结构化输出；支持与 `enable_thinking=true` 组合
  - `/api/chat/` 与 `/api/chat/stream` metadata 新增 `enable_structured_output`、`requested_structured_output`、`response_schema_name`
- Gemma4 思考/推理模式对齐官方实现（vLLM reasoning_content）：
  - 非流式：后端直接提取 OpenAI-compatible 响应中的 `reasoning_content` 与 `content`，不再依赖拼接 `thought ... Final answer` 文本
  - 流式：`/api/chat/stream` 新增 SSE `reasoning` 事件（思考 token）与原 `token` 事件（回答 token）
  - `done` 事件与 `/api/chat/` metadata 持续输出 `reasoning_content`、`final_content`、`display_content`
  - 前端优先消费结构化字段与 `reasoning` 事件，保留旧 `thought` 文本格式的兼容兜底
- Tool Calling 轨迹可视化（Agent 执行链路）：
  - 后端工具调用返回结构新增 `tool_traces`（含工具名、参数、输出、耗时、时间戳）
  - `/api/chat/stream` 新增 SSE `tool_trace` 事件，`done` 与非流式 metadata 同步包含 `tool_traces/tool_trace_count`
  - 会话历史落库新增 `tool_traces`，前端可折叠查看“工具执行轨迹”
- 新增“对话附件上传并直读”能力（你要求的“像图片一样上传文件给 AI 读”）：
  - 聊天请求支持文件字段：`file/file_name/file_format`
  - 后端支持 `txt/md/pdf/csv/json/log` 解析并注入当次对话上下文
  - 若附件是音频（重点支持 `mp3`），会自动切换到 Gemma4 原生音频理解链路（不走文本文件解析）
  - 前端聊天框附件按钮支持文本附件与 `mp3`，其中 `mp3` 会自动按音频上传流程处理
...
# 音频发送前自动转 wav，提升 mp3 在不同 vLLM/Gemma4 组合下的兼容性
CHAT_AUDIO_TRANSCODE_TO_WAV=true
FFMPEG_BINARY=ffmpeg
AUDIO_TRANSCODE_TIMEOUT_SECONDS=30
...
# 说明：mp3 等音频附件会自动走音频链路（受 CHAT_AUDIO_ALLOWED_EXTENSIONS 控制），不受该项限制
...
说明：`deploy/vllm/start_vllm.sh` 支持离线本地模型优先启动；可通过 `MODEL_PATH=/abs/path/to/model FORCE_LOCAL_MODEL=1` 绕过 Hugging Face 在线拉取，适用于 `Network is unreachable` 场景。
...
结构化输出请求体示例（`response_format=json_schema`）：
...
使用方式：
- 在聊天输入框左侧点击回形针按钮上传文件（`txt/md/pdf/csv/json/log/mp3`）。
- 选择 `mp3` 时会自动走 Gemma4 音频理解链路（等价于音频上传），不会按文本附件解析。
- 可只发文件不输入文本，后端会自动补全默认提问。
- 文本附件内容仅用于当前请求上下文，不会自动写入 RAG 知识库。
...
## 模式能力（Thinking / Tool Calling / Structured Output）

你现在可以在聊天框中按请求开关：
- `Thinking`：对应请求参数 `enable_thinking=true`
- `Tool Calling`：对应请求参数 `enable_tool_calling=true`
- `Structured Output`：对应请求参数 `response_format={"type":"json_schema",...}`

说明：
- Thinking / Tool Calling 是请求级别开关。
- Structured Output 同样是请求级别开关（当前仅 `LLM_PROVIDER=vllm` 支持）。
- 需要服务端 vLLM 启动时允许相关能力（例如 `ENABLE_REASONING=1`、`ENABLE_TOOL_CALLING=1`）。
- 重要：`response_format` 只约束 JSON 结构，不会把 schema 描述文本“喂给模型”；语义规则仍需写在 `system`/提示词里。
- 当前 Tool Calling 内置工具：
  - `get_current_time`（时间查询）
  - `get_beijing_time`（北京时间查询，固定 Asia/Shanghai）
  - `math_calculator`（基础数学表达式计算）
  - `get_weather`（城市实时天气，使用 Open-Meteo）
  - `calculate_wind_chill`（风寒指数计算）
  - `search_web_realtime`（实时网页新闻检索，默认按时间倒序；provider 优先 Bing，失败自动尝试 Google，再兜底 Baidu）
  - `search_news`（兼容别名：优先走实时检索链路（Bing/Google/Baidu），失败时回退 HN）
  - `get_exchange_rate`（汇率查询与金额换算）
  - `stock_quote`（股票/ETF 实时报价）
  - `convert_unit`（常用单位换算：温度/长度/重量）
  - `get_calendar_info`（日期/星期/ISO 周信息）
  - `text_stats`（文本统计：字数/词数/行数等）
  - `generate_uuid`（生成 UUID）
  - `random_number`（随机整数）
  - `coin_flip`（抛硬币）
  - `roll_dice`（掷骰子）
  - `generate_password`（本地随机密码）
  - `hash_text`（文本哈希）
  - `base64_codec`（Base64 编解码）
- `search_web_realtime` 参数建议：
  - `query`：关键词（必填）
  - `freshness`：`hour/day/week/month/any`（默认 `day`）
  - `limit`：返回条数（1-10）
  - `market/language`：区域与语言（默认 `en-US`）
- Gemma4 function tool 标注示例（OpenAI-compatible）：
...
```json
{
  ...
  "messages": [
    {
      "role": "user",
      "content": "请用工具查询 OpenAI 过去24小时的最新新闻"
    }
  ],
  "tools": [
    {
      ...
      "function": {
        ...
        "parameters": {
          ...
          "properties": {
            "query": { "type": "string" },
            "freshness": { "type": "string", "enum": ["hour", "day", "week", "month", "any"] },
            "limit": { "type": "integer" }
          },
          "required": ["query"]
        }
...
```
- 相关配置（`backend/.env`）：
  - `MAX_TOOL_CALL_ROUNDS`：单次请求允许的工具调用轮数上限（默认 4）
  - `WEATHER_TOOL_ENABLED`：是否开启天气工具（默认 true）
  - `WEATHER_TOOL_TIMEOUT_SECONDS`：天气/联网工具 HTTP 超时（默认 12 秒）
...
- `Profile: xxx` 标签可点击查看六档能力说明（Image/Audio/Video/Thinking/Tool Calling/Structured Output）。
