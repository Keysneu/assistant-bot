...
const FILE_HINT = ".txt .md .markdown .html .htm .pdf";
...
export function DocumentManager({ onDocumentsChange }: DocumentManagerProps) {
  ...
  const handleRemovePendingFile = (target: File) => {
    const targetKey = fileKey(target);
    setPendingFiles((prev) => prev.filter((file) => fileKey(file) !== targetKey));
  };
  ...
  const handleBatchUpload = async () => {
    ...
    try {
      ...
      const failed = result.documents.filter((doc) => doc.status.startsWith("failed"));

      if (result.success_count > 0) {
        setSuccessMsg(
          `上传完成：成功 ${result.success_count}/${result.total_files}，新增 ${result.total_chunks} 个知识片段`
        );
        await loadDocuments();
        onDocumentsChange?.();
      }
      ...
      if (failed.length > 0) {
        const failedNames = failed.map((item) => item.filename).join("、");
        setError(`部分文件上传失败：${failedNames}`);
      }
            ...
            <>
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 animate-in fade-in-0 slide-in-from-bottom-4">
                {paginatedDocuments.map((doc, idx) => {
                  const isUrlDoc = doc.file_type === "url" || doc.file_type === "web";
                  const displaySource = doc.source.replace("file://", "");
                  return (
                    <Card
                      key={doc.document_id}
                      ...
                      <CardContent className="p-4 flex flex-col h-full justify-between gap-4">
                        <div className="flex items-start gap-3">
                          <div className="p-2 rounded bg-primary/10 text-primary">
                            {isUrlDoc ? <Globe className="h-5 w-5" /> : <FileText className="h-5 w-5" />}
                          </div>
                          <div className="min-w-0 flex-1">
                            <h4 className="font-medium text-sm truncate" title={displaySource}>
                              {displaySource}
                            </h4>
                            <p className="text-xs text-muted-foreground mt-1">
                              {doc.chunk_count} 片段 ·
                              {doc.created_at ? ` ${new Date(doc.created_at).toLocaleDateString()}` : " 未知时间"}
                            </p>
                          </div>
                        </div>
        ...
        <Dialog open={deleteConfirmOpen} onOpenChange={setDeleteConfirmOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>此操作无法撤销。这将永久删除该文档及其所有知识片段。</DialogDescription>
            </DialogHeader>
            <DialogFooter>
        ...
        >
          <DialogContent className="sm:max-w-[560px]">
            <DialogHeader>
              ...
              <DialogDescription>上传文件或抓取网页内容以扩充知识库</DialogDescription>
            </DialogHeader>
            ...
            {uploadMode === "file" ? (
              <div className="space-y-4">
                <div
                  className={cn(
                    "flex flex-col items-center justify-center py-8 border-2 border-dashed rounded-lg bg-muted/20 transition-colors",
                ...
                >
                  ...
                  <h3 className="text-lg font-semibold mb-1">点击或拖拽上传多个文件</h3>
                  <p className="text-sm text-muted-foreground mb-6">支持 {FILE_HINT}</p>
                ...
                {pendingFiles.length > 0 && (
                  <div className="space-y-2 max-h-44 overflow-auto rounded-md border p-3">
                    {pendingFiles.map((file) => (
                      <div key={fileKey(file)} className="flex items-center justify-between gap-3 text-sm">
